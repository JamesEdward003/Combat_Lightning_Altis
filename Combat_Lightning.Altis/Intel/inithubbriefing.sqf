...
BIS_HubBriefing_setupBriefing =
{
	...
	*/
	...
	_fn_getSnappoint =
	{
		...
		_chairs = [nearestObject [_this,"Land_CampingChair_V2_F"],nearestObject [_this,"Land_ChairWood_F"]];        //changed to support simple objects
		_chairs = _chairs select {!isNull _x && {_x distance _this < 0.65}};
	...
	_fn_setupUnit =
	{
		...
		if (isNull _unit) then
		{
			...
			[
				"FGN_FIA_W_Rifleman",
				"FGN_FIA_W_Driver",
				"FGN_FIA_W_RiflemanPBG",
				"FGN_FIA_W_Marksman",
				"FGN_FIA_W_AutomaticRifleman",
				"FGN_FIA_W_RiflemanM21",
				"FGN_FIA_W_RiflemanM16"
			];
			};
	...
	//make sure the key down state is reset
...

	{
		if (true) then
		{
			...
			if (_requiredCount > 1) then
			{
				if (_forHint) then
				{
					_requiredText = _requiredText + format["<br/><img color='#ffffff' image='%4'/> %2 (%1x)",_requiredCount,_displayName,_iconPath,_checkbox];
				}
				else
				{
					_requiredText = _requiredText + format["<br/><img color='#ffffff' image='%4'/> %2 (%1x)",_requiredCount,_displayName,_iconPath,_checkbox];
			...
			else
			{
				if (_forHint) then
				{
					_requiredText = _requiredText + format["<br/><img color='#ffffff' image='%4'/> %2",_requiredCount,_displayName,_iconPath,_checkbox];
				}
				else
				{
					_requiredText = _requiredText + format["<br/><img color='#ffffff' image='%4'/> %2",_requiredCount,_displayName,"%2",_checkbox];
				};
			};
		};
	}
...
BIS_HubBriefing_exec =
{
	...
	*/
	...
	if !(BIS_skipBriefingCutscene) then
	{
		...
		[
			"KeyDown",
			{
				...
				if (!BIS_HubBriefing_skip && {_this select 1 == 57 && {time > (missionNamespace getVariable ["BIS_HubBriefing_skipTime",0])}}) then
				{
					BIS_HubBriefing_skipTime = time + 1;
		...
		if !(BIS_HubBriefing_skip) then
		{
			...
			if (_skipTimeAllowed) then
			{
				//Camp Maxwell location is not properly defined on Stratis, will force the proper string
				if (missionName == "A_hub01" || {missionName == "A_hub02"}) then
				{
					[getPosASL player,date,"",localize "str_a3_campmaxwell0"] spawn BIS_fnc_camp_showOSD;
				}
				else
				{
...

	{
		...
		if (_state == "CREATED" || {_state == "ASSIGNED" || {_state == "AUTOASSIGNED"}}) then
		{
		...
		while {(!BIS_HubBriefing_timeOver)} do
		{
			if (!BIS_HubBriefing_timeOver) then
			{
				...
				if (_location == "ARMORY") then
				{
					...
					{
						[player,"BIS_GoArmory",[_taskArmoryDesc,_taskArmoryTitle,_taskArmoryMrk],[_armoryPoint,true],true,nil,true,"armor",true] call BIS_fnc_taskCreate;
					}
					else
					{
						//["BIS_GoArmory",[_taskArmoryDesc + _requiredGearTextTaskCurrent,_taskArmoryTitle,_taskArmoryMrk]] call BIS_fnc_taskSetDescription;
				...
				else
				{
					...
					if !("BIS_Assemble" call BIS_fnc_taskExists) then
					{
						[player,"BIS_Assemble",[_taskAssembleDesc,_taskAssembleTitle,_taskAssembleMrk],[_assemblyPoint,true],true,nil,true,"move",true] call BIS_fnc_taskCreate;
					}
					else
					{
