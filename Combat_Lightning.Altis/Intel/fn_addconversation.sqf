...
_firstSentence = "<img size='2' shadow='2' image='\a3\ui_f\data\igui\cfg\Actions\talk_ca.paa'/> '" + (format ([_firstSentence] + _arguments)) + "'";
_id = _addTo addAction [_firstSentence, {nul = [(_this select 1), (_this select 3 select 0), (_this select 3 select 1)] spawn IP_fnc_openConversation}, [_unit, _conversation], 1.5, false, true, "", ("(_this distance _target < 3) && " + _condition)];
