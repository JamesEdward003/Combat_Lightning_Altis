/*
 * Author: Josef Zemanek & PierreMGI
 *
 ...
 *
 * Example:
 * [] execVM "fn_CPInit_mgi.sqf";
 *
 * Public: No
 */
if ( ("BI_CP_Version" call BIS_fnc_getParamValue) isEqualTo 1 ) exitWith {};

//deleteVehicle CP_Module;
...
if !(isDedicated) then {
  ...
  if (didJIP && (("BI_CP_Version" call BIS_fnc_getParamValue) isNotEqualTo 1)) then {
    _null = [] spawn {
    ...
    _null = [] spawn {
      ...
      while {BIS_CP_targetLocationID == -1} do {
        ...
        if ((missionNamespace getVariable ["BIS_CP_voting_countdown_end", 0]) > 0) then {
          ...
          } forEach ((units GAMBLER + units SCRAMBLER) select {isPlayer _x});
          _mostVoted = 0;
          _mostVotes = 0;
...
private _armada = [];
if ({isplayer _x} count units GAMBLER > 0) then {{_armada pushBack _x} forEach units GAMBLER};
if ({isplayer _x} count units SCRAMBLER > 0) then {{_armada pushBack _x} forEach units SCRAMBLER};
  {
  ...
  _playersNo = {isplayer _x} count (units GAMBLER + units SCRAMBLER);
...
_null = [] spawn {
  if (isServer) then {
    ...
    [{{_x distance BIS_CP_exfilPos < 30 && isPlayer _x && alive _x} count (units GAMBLER + units SCRAMBLER) == ({isPlayer _x && alive _x} count (units GAMBLER + units SCRAMBLER)) && {alive _x} count (units GAMBLER + units SCRAMBLER) > 0}, 1] call BIS_fnc_CPWaitUntil;
    ...
    if (missionNamespace getVariable ["BIS_CP_objectiveDone", FALSE]) then {
      ...
      if ({!alive _x} count (units GAMBLER + units SCRAMBLER) > 0) then {
